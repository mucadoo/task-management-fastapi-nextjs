    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {date && (
            <X
              className="h-4 w-4 text-warm-500 hover:text-red-500 cursor-pointer"
              onClick={clearAll}
            />
          )}
        </button>
      ...
      <PopoverContent className="w-auto p-0" align="start">
        ...
        <div className="border-t border-warm-100 dark:border-white/10 p-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <input
              type="time"
              className="flex h-9 w-[100px] rounded-md border border-warm-200 dark:border-white/10 bg-white dark:bg-[#141414] px-2 py-1 text-sm text-warm-900 dark:text-gray-100 shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-brand-500 disabled:cursor-not-allowed disabled:opacity-50"
            ...
            {hasTime && (
              ...
              >
                <X className="h-3 w-3 text-warm-500 hover:text-red-500" />
              </button>
            )}
          </div>
        </div>
