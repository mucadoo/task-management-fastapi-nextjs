...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-5/12 bg-brand-600 dark:bg-brand-900 p-12 flex-col justify-between" style={{ backgroundImage: "repeating-linear-gradient(45deg, rgba(255,255,255,0.03) 0, rgba(255,255,255,0.03) 1px, transparent 0, transparent 50%)", backgroundSize: "20px 20px" }}>
        <div>
          ...
          <div className="card-surface p-6">
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <button type="submit" disabled={isLoading} className="btn-primary w-full mt-2">
                {isLoading ? <LoadingSpinner size="sm" className="text-white" /> : <>{t('auth.register')}<ArrowRight className="h-4 w-4" /></>}
              </button>
            </form>
          </div>
        </div>
