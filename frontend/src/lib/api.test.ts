...
describe('api', () => {
  ...
  describe('tasks', () => {
    it('creates a task', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/tasks/'), expect.objectContaining({ method: 'POST' }));
    });
    ...
    it('toggles task status', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/tasks/1/toggle'), expect.objectContaining({ method: 'POST' }));
    });
  });
  ...
  describe('auth', () => {
    ...
    it('stores tokens on register', async () => {
      const mockTokens = { access_token: 'access_reg', refresh_token: 'refresh_reg', token_type: 'bearer' };
      (fetch as any).mockResolvedValue({
        ok: true,
        json: async () => mockTokens,
      });
    ...
    it('updates current user', async () => {
      ...
      expect(result).toEqual(mockUser);
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/auth/me'), expect.objectContaining({ method: 'PATCH' }));
    });
  });
