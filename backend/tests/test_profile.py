...
def test_update_me_name(client: TestClient, auth_headers: dict):
    response = client.patch("/api/auth/me", json={"name": "New Name"}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
def test_update_me_email(client: TestClient, auth_headers: dict):
    ...
    response = client.patch("/api/auth/me", json={"email": new_email}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
def test_update_me_password(client: TestClient, auth_headers: dict):
    ...
    response = client.patch("/api/auth/me", json={"password": new_password}, headers=auth_headers)
    assert response.status_code == 200
