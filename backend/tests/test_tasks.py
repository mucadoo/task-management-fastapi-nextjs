...
def test_task_ownership_isolation(client, auth_headers, second_user_auth_headers):
    ...
    client.post("/api/tasks/", json={"title": "User 1 Task"}, headers=auth_headers)
    ...
    user1_task_response = client.post(
        "/api/tasks/", json={"title": "User 1 Task"}, headers=auth_headers
    )
    ...
    response = client.post(
        f"/api/tasks/{task_id}/toggle", headers=second_user_auth_headers
    )
    assert response.status_code == 404
...
def test_toggle_task_status(client, auth_headers):
    create_response = client.post(
        "/api/tasks/", json={"title": "Toggle me"}, headers=auth_headers
    )
